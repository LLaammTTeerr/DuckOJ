...
export class TeamAccessService {
  ...
  async update(
    ...
    if (memberIds !== undefined) {
      // F-25: a roster change is refused while the team is in a running contest
      // (unless the caller runs every such contest). B-18: and it must never
      // put one pupil on two rows of one board.
      await this.assertRosterUnlocked(actor, team.id);
