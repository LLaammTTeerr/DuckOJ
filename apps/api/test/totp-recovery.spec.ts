  ...
  it('reports the remaining count on GET /auth/me, and decrements it as codes are spent', async () => {
    await withTestDb(async (db) => {
      ...
      try {
        ...
        expect((await agent.get('/api/v1/auth/me')).body.recoveryCodesRemaining).toBe(8);

        const used = await login(app, 'rcbeta', { recoveryCode: codes[0] });
        expect(used.status).toBe(200);
        expect(used.body.user.recoveryCodesRemaining).toBe(7);
        expect((await agent.get('/api/v1/auth/me')).body.recoveryCodesRemaining).toBe(7);
      } finally {
        await app.close();
      }
    });
  ...
  it('notifies the holder when the last code goes', async () => {
    await withTestDb(async (db) => {
      ...
      try {
        ...
        expect((await agent.get('/api/v1/notifications')).body.items).toHaveLength(0);

        expect((await login(app, 'rceta', { recoveryCode: codes[7] })).status).toBe(200);
        const feed = await agent.get('/api/v1/notifications');
    ...
    await withTestDb(async (db) => {
      ...
      try {
        ...
        const again = await agent
          .post('/api/v1/auth/totp/recovery/regenerate')
          .send({ code: authenticator.generate(secret) });
  ...
  it('refuses a wrong code, and refuses outright when 2FA is off', async () => {
    await withTestDb(async (db) => {
      ...
      try {
        ...
        const wrong = await agent.post('/api/v1/auth/totp/recovery/regenerate').send({ code: '000000' });
        expect(wrong.status).toBe(422);
        expect(wrong.body.code).toBe('invalid_totp_enrolment_code');

        const plain = request.agent(app.getHttpServer());
        await plain.post('/api/v1/auth/register').send({
          username: 'rckappa',
          email: 'rckappa@example.com',
          password: PASSWORD,
          displayName: 'rckappa',
        });
        await plain.post('/api/v1/auth/login').send({ usernameOrEmail: 'rckappa', password: PASSWORD });
        ...
        const none = await plain.post('/api/v1/auth/totp/recovery/regenerate').send({ code: '000000' });
  ...
  it('leaves no rows behind, so a re-enrolment cannot inherit a stale printout', async () => {
    await withTestDb(async (db) => {
      ...
      try {
        ...
        expect((await agent.delete('/api/v1/auth/totp').send({ password: PASSWORD })).status).toBe(204);
