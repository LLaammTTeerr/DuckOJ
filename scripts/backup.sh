...
#
# Env overrides:
#   KEEP=14           how many backups to keep (0 disables pruning)
...
# COMPOSE_PROJECT=duckoj (or PG_CONTAINER) when running from anywhere but the
# real checkout.
#
...
if [ -z "$PG_CONTAINER" ]; then
  ...
  echo "       Set COMPOSE_PROJECT or PG_CONTAINER (see the header of this script)." >&2
  exit 1
fi
