...
Left open, and **closed by D205**: `live` is still keyed by job id, not
`(job, attempt)`. Terminating attempt N on its own connection and not reusing
that connection until the judge answers narrows the window hard, but it is
still an argument from timing. D205 keeps the map's key and moves the attempt
onto the connection assignment instead, which is what the driver actually
routes on.
...
## D139 — The desktop bar collapses the account PAGES, never the way out
...
## D149 — A container-backed spec inherits its timeout from the package, and a guard says so
...
## D150 — A candidate bundle is vetted from a real, allow-listed origin — configuration, not a lie about `Origin`
...
## D204 — A judge's token is rotated by `rotate`, and the poll that guards a live connection asks about the credential, not the name

F-58 struck `docs/guide/truoc-khi-trien-khai.md` §1's rotation instruction —
`judge:node revoke judge-1` then `add judge-1` — because it cannot work.
`addJudgeNode` refuses a name it already holds:

```
judge node 'judge-1' already exists — revoke it, or pick another name
```

and `revoke` keeps the row on purpose, so the name is never free again. D68
recorded the intended workaround in as many words ("Rotation is `revoke` then
`add` under a new name"), and step 1 of the province's one-time checklist did
not follow it: it re-used the name. **The sequence burned the one credential
this repository's history has already leaked, and then failed** — a province
left with a dead judge and no way forward except hand-typed SQL. The refusal
has been there since `c8a4672`, the commit that created the CLI; the checklist
page is a descendant.

That credential is not hypothetical. It is in this repository's history, in
`.env.example`'s shape, and on every machine that has ever run this stack. A
province that cannot rotate it is running a judge that anyone who read the
repo can impersonate.

### The ruling, in two halves

**`judge:node rotate <name>` mints a new token for a node that exists.** The
row is kept, exactly as `revoke` keeps it — `grading_jobs.judge_node_id`
references `judge_nodes` `on delete set null`, so the machine that graded each
submission stays named across a rotation, which is the point of rotating
rather than retiring: it is the SAME judge with a new key. The token is
generated by the command and printed once; there is no `--token`, for D68's
reason (argv is shell history).

**It works on a REVOKED node, and says so.** This is deliberate and it is the
part that matters most today: a province that already followed the struck
instruction is standing in the dead end *now*, and `rotate` is the only exit
that does not involve SQL. Un-burning a credential somebody deliberately
refused is loud — the CLI prints `judge node 'x' was revoked — this rotation
RE-ADMITS it` above the token.

**And D81's five-second poll now matches the credential, not the name.** This
half is what makes the first half safe rather than dangerous. `verifyJudge`
runs exactly once, at the handshake; D81 added a poll so `revoke` could reach
a judge already holding a socket, and it asked `admittedJudgeNames(ids)` —
"does this name still have an unburned token". That is the right question for
`revoke` and the **wrong** one for `rotate`: a rotated row is neither gone nor
burned, so the old judge would have answered "still admitted" — kept its
connection, kept being dispatched to, and been 401'd by the API on every
package fetch, because the API's package guard re-verifies per request.
**That is exactly the half-working judge D81 was written to kill, reached from
the other direction**, and every submission sent to it would fail instead of
being graded.

So `BridgeServer` keeps `hashJudgeToken(key)` per connection — the digest,
never the key, and hashed through `@duckoj/db`'s own function rather than a
second `createHash` — and the poll is `admittedJudgeCredentials(pairs)`,
matching `(name, token_hash)` in SQL. `admittedJudgeNames` is retired rather
than kept beside it: two functions asking almost the same question is how one
of them drifts. Revoke, rotate and a row an operator deleted by hand collapse
into **one rule: a connection whose credential is no longer the stored
credential goes.** A revoked row needs no special case — `revoked:<old hash>`
equals no digest any connection could be holding — so the `not like` predicate
D81 needed is gone.

The comparison stays in the database. A token-hash digest is a credential and
has no business crossing the wire on a five-second timer so that a `===` can
run in Node.

### What a rotation costs, stated rather than implied

- **The connected judge is dropped within five seconds**, through `retire` —
  the same path as a judge that died, so whoever was grading on that socket
  hears about it.
- **The in-flight submission is requeued, not failed.**
  ...
  being on the judge that was rotated would be B2's failure shape all over
  again. It is graded again when a judge returns.
- **The queue parks; it does not reject.** With no judge connected, dispatch
  waits rather than failing (D68's deliberate "an empty fleet still parks").
  The cost of a rotation is latency.
- **The operator MUST recreate the judge container, not restart it.** The
  judge reads `JUDGE_TOKEN` from its own environment — `judge/entrypoint.sh`
  renders `judge/judge.yml` from it at start — and a container's environment
  is fixed when the container is created. `podman restart judge` brings back
  the same dead token. `podman-compose up -d judge` (or
  `scripts/compose-up.sh`) is the step.
- **Between the rotation and the recreate, `judged` logs one
  `judge handshake rejected` per redial.** That loop is the proof the old
  token is refused, not a fault, and it ends when the recreate lands.

The log line D81 wrote on a drop was `dropping revoked judge`; it is now
`dropping judge no longer admitted`, because "revoked" is false for a rotation
and would send an operator hunting a revocation nobody made.

### Alternatives

| Alternative | Why not |
| --- | --- |
| **Leave rotation as `revoke <old>` + `add <new name>`** (D68's answer) | It works, and it is not what a province needs: the new name has to be pushed into `JUDGE_NAME` as well as `JUDGE_TOKEN`, `judge_nodes` accumulates a dead row per rotation, and — the actual blocker — it does not rescue anybody who already ran the struck sequence and burned the name they still have to use. |
| **Let `add` silently rotate an existing name** | D68 refused this and was right: re-registering a name is far more often a typo than an intention, and a typo that invalidates a running judge's credential is a stack-wide outage that looks like a network problem. A separate verb is one word longer and cannot be typed by accident. |
| **Rotate, and leave the live connection alone** | The zombie above. It is worse than dropping the judge, because the fleet looks healthy while every submission it takes fails. |
| **Refuse to rotate a connected judge** | It would make the command useless for the only case it exists for. A judge is connected essentially always. |
| **Refuse to rotate a REVOKED node** | Tidier, and it strands exactly the province this decision exists to unstrand. |
| **Have `rotate` restart the judge itself** | A database CLI that reaches for the container runtime. It cannot work for a judge on another host, which is the fleet D68 built for, and it would put a fleet-wide outage one typo away from a command an operator runs at a shell. |
| **A `token_rotated_at` column and a migration** | The digest the bridge already holds answers the question with no schema change, and a timestamp answers it worse: it cannot tell a connection holding the new token from one holding the old. |

*Ruled by the implementer during the F-59 slot, no human available to consult.
No migration. Cost if wrong: `judge:node`'s verb set, one `@duckoj/db` export,
