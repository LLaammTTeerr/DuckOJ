...
**DONE_WITH_CONCERNS.** Branch `worktree-agent-a47e6d8ec92ad46a0`, 5 commits,
not pushed: `ead257f` API · `6f1dc80` tests · `6ebe009` web + D71 ·
`3bb63eb` test hardening · `fe69808` CORS.

## Shipped
Three routes on `ContestsController`, tag `Contests`, all gated on
...
filter (1). Two of them exposed **assertions
...
Ritual green on the final pass: api 792/792 (89 files), web 340/340, every
other package green, typecheck + lint + `typecheck:scripts` + `lint:scripts`
clean, regen leaves no diff, `vite build` OK.
...
`end_time`, anyone" would leak what D22/D23 hide, and the brief's "after the
end" lives in the web instead.
...
- **Deviation:** `authz/**` is outside the touch list, but `org_members` is
  ...
  `contest.access.ts`, no module registration. The in-scope alternative was an
  org column identical on every row: wrong data, not merely less.
...
- Full suites flaked hard under machine load: three runs failed a *different*
  set of first-in-file tests, all green in isolation.
