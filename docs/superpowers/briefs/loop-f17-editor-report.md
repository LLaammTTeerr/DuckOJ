...
- `src/editor/code-editor.tsx` — CodeMirror 6 by hand (line numbers, history,
  ...
  `aria-label` + `id="source"`; compartments for grammar and font size; an
  `External` annotation so the page's own writes are not read as typing.
...
- `app.css` — `--syn-*` scale (light+dark) on `tok-*`, editor chrome, 22rem
  desktop / 40vh at D76's 700px breakpoint; `i18n/{en,vi}.ts`: 8 keys each.

## Tests — 462 web unit tests green (`vitest run --no-file-parallelism`), 12
new in `test/editor.spec.tsx`; `test/setup.ts` stubs `ResizeObserver`, `Range` rects,
...
stored draft → restore red; `External` guard dropped → starter filed as draft.
...
only `#source` site). e2e NOT run: needs the live stack, off-limits here.

## Bundle
`code-editor` chunk **505.36 kB (171.30 kB gzip)**, fetched only when a submit
form renders; entry 1,099.40 → 1,105.08 kB (gzip 332.90 → 334.52), measured by
building the pre-F17 tree and this one side by side.
...
4. No hidden `<textarea>` twin: a second copy of the buffer is a correctness
   hazard, not an accessibility win. 5. Size in UTF-16 code units, matching
   Zod, not bytes. Left out: no persisted editor settings, no completion.
