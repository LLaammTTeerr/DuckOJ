...
4. **`JUDGED_CONCURRENCY` ships 1** (`config.ts`, `docker-compose.yml`, `.env.example`; ruling 1). **D29**
...
2. **One-submission-per-connection is a ruling, not a confirmed fact** — argued in D29 from the protocol
...
3. **Fast-forwarded the worktree branch to `main`** (2 commits, clean ff) — `final-review.md` was not in the
   branch. Committed on the worktree branch per the dispatch. **D29 skips D26–D27** (DECISIONS.md ends at D25).
