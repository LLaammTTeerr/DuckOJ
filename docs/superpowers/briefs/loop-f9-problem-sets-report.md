...
Six commits, one migration (**0026**), **D66**. Ritual: typecheck (incl. scripts),
lint, regen no-diff, `vite build`, and both suites green run on their own (see
Concerns). 29 mutants, all killed.
...
`ProblemSetAccessService` asks `OrgAccessService` who may act, through
`loadVisibleWithRole` (new) and `loadForEdit` (now public).
...
teacher's grid on its own route, CSV link straight at the API, inside the `tabindex`
scroll wrapper m21 asked for (`.grid-scroll`, sticky header); an assign/edit form
with a problem picker. vi/en.

## Rulings (all in D66)
- **Member-gated, blanked never signalled.** A non-member of a *visible* school gets
  an empty list and `problem_set_not_found` from every set, not a 403 — an item may
  name an `org` problem shared with that school alone.
...
- **422 at write time** for a problem the school could not open (`…_private`,
  another school's `org` problem included), an unknown code or a repeat, keyed
  `problems[<n>].code`. One narrowed *afterwards* keeps its row, `visible: false`.
  **D35 has nothing to mask**: an item carries code, name and points only.
...
concurrently**, and two unrelated web specs (different ones on each of two runs —
`logout`, `orgs`, `contest-freeze`) flake on `waitFor` under that contention; both
suites pass run on their own.
