...
247 files, ~45.8k insertions; every non-generated source diff read. Seven defects
confirmed, fixed, mutation-checked where behavioural, one commit apiece. Rulings
...
**every 30 s for every reader**. Capped at 200 newest + `truncated`. **d8a7166** (D63)
...
exposed three headers (e04d056); F-6 shipped two more two commits later and left them off.
The new assertion derives the list from source. **c7c686a**

**5.** `authn/expired-rows.sweeper.ts:88` — the sweep materialised every deleted id; its own
header estimates `rate_events` at ~8.6M rows/day under stuffing. Driver count now. **acaafdb**
...
**7.** `apps/api/test/redis.harness.ts:32` — three cache specs `flushall` one shared
container, so under whole-suite load two wiped each other mid-assertion (the ritual went
red on exactly that). One logical db each, `flushdb`. **527ef9f**

## Recorded, not fixed

- `authz/dashboard.access.ts:263` — `workers()` LEFT JOINs `grading_jobs` to `submissions`
  with no time bound, every 15 s: worse than D47's "aggregates over `grading_jobs`".
- The `{ data } = await api.GET(…)` swallow survives at 9 query functions despite B-4's
  "every read query carries its status"; two are new — `web/src/tags.ts:20`, and
  `web/src/routes/orgs.tsx:230` (a school's contests read as "none" on a failure).
- `apps/api/test/route-fuzz.spec.ts` holds raw NUL bytes (5114, 6136, 6249) so git treats it
  as **binary** and its diff is unreviewable; use escapes. `markdown-to-typst.ts:291`
  hardcodes `BOOKLET_TZ` to ICT beside D57's per-account timezone (fine for print).

**Cleared:** migrations 0016–0023 (no drift; the 0020 gap is harmless); contracts/SDK
regen (no diff); i18n parity; route markers; `assertMayJoin` (intersection, not whole
membership); every date call site passing the D57 zone; D53's cap on every unpack.
