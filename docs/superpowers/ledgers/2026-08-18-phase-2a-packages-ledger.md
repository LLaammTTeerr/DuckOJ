...
| `packages/contracts/src/registry.ts:9` hardcodes `/api/v1` as the OpenAPI `servers` URL, outside `@duckoj/api-prefix`'s single-sourced `API_PREFIX` | Task 13 minor (deferred) |
...
Worktree: .claude/worktrees/phase-2a-packages on branch worktree-phase-2a-packages, branched from 5debc64.
Baseline entering the plan: 181 tests, all gates green, Phase 1 merged to main at 494555b plus 070331b (test build ordering).
...
Both fixed in the plan before Task 1 (commit 317db4f).
...
Task 1: complete (commits 317db4f..bfd4c5f, review clean).
...
Task 2 fix round 1: implementer DONE (commit 0180b32). Both guards added with field-naming errors, golden-vector test pinned, 196/196 (186 + 10). Reports identity-preservation confirmed and that changing `\0` to `|` fails the golden test while the other 14 pass. Scoped re-review dispatched (haiku) with three things it must reproduce rather than accept: the two collision constructions actually throwing, the identity-preservation claim checked by hashing a normal fixture against `git show 1813b81:...hash.ts`, and the golden vector's expected digest being a hardcoded literal rather than a value the test computes by calling the code under test — which would restore precisely the blind spot the fix exists to close.
...
Task 2: complete (commits bfd4c5f..0180b32, review clean after 1 fix round).

Task 3: implementer DONE (commit c9c70bb). 200/200 (196 + 4), typecheck and lint green across all 10 projects.
...
Task 3: complete (commits 0180b32..c9c70bb, review clean).
...
Task 4 — the pattern I was hunting **did exist**, attached to the other deviation. Intermediate commit 00151aa worked the flatMap type error around with `Array<Record<string, any>>` and `doc: any`, commented "to work around strict type checking". It was fully cleaned up in fef7b7c, and the reviewer confirmed that cleanup was forced rather than cosmetic: `@typescript-eslint/no-explicit-any` is an error in the shared config, so the `any` version would have failed `pnpm lint`. Resolved history, not a live finding — but it is a good illustration that a false rationale in a report usually has a real one nearby.
...
Task 5: complete (commits ccaef72..059ef7c, review clean).
...
Task 6: complete (commits 059ef7c..8b2c05c, review clean).
...
Task 7 fix round 2: implementer DONE (commit 53d1eb2), 218/218. It agreed with the reasoning before applying rather than complying silently, which is what I asked for.
...
Task 7: complete (commits 8b2c05c..53d1eb2, review clean after 2 fix rounds).

Task 8: implementer DONE_WITH_CONCERNS (commit f945943). 222/222, all gates green. Bridge handshake now awaits `verifyJudge` before registering or replying, fails closed on `false` or a throw, and Phase 1's displaced-connection handling is preserved. Both sides delegate to one shared `verifyJudgeCredential(db, name, token)` in `@duckoj/db` rather than duplicating — the implementer applied this plan's own Ruling P2 precedent unprompted.
...
Task 8: complete (commits 53d1eb2..21460db, review clean after 1 fix round). 223 tests.
...
Task 11: complete (commits 1073c8d..09efbac, review clean, no findings).

## STOPPED — session limit (resets 20:50 Asia/Ho_Chi_Minh)
Task 12's implementer terminated on an API session limit before doing any work. Verified: HEAD is 09efbac, working tree clean, nothing partial. Task 12 has NOT started.
Resume point: dispatch Task 12 with brief `.superpowers/sdd/2026-08-18-phase-2a-packages/task-12-brief.md` (addendum B1/B2 already appended), BASE 09efbac.
...
Task 13 fix round 1 re-review: ADDRESSED. Discrimination reproduced exactly — changing the prefix to `api/v2` failed **both** smoke assertions with `expected 404 to be 401`, then restored green. All three consumers confirmed importing `@duckoj/api-prefix` with no re-declaration.
...
Task 13: complete (commits 3509f16..df1827e, review clean after 1 fix round). 256 tests.
...
```
`ls /problems/` inside the judge container shows **both** hashes — `0f61232b…` (hello) and `73d40a7e…` (aplusb). Commit 914c688, 256/256, all gates green.
...
Task 14: complete (commits df1827e..914c688, review clean).
...
Task 15: complete, commit `dc1fe7d` (docs, runbook, e2e-submit.ts comment,
and this ledger; `914c688` remains the last code change). **256/256, all
