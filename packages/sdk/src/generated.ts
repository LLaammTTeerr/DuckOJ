...
export interface paths {
    ...
    "/contests/{key}/seats.pdf": {
        ...
        /**
         * Printable seat slips, one card per competitor — the organisers only
         * @description An A4 portrait sheet of eight cards on dashed cut lines (D129). Each card carries the contest’s name, the competitor’s display name, their account — for a team, every member’s username instead (D99) — the contest window in the caller’s own timezone (D57/D64, offset derived rather than hardcoded), the site’s address, and a blank rule for the room and seat number. **No password appears on a slip and none can**: a roster import returns its credentials exactly once and stores only hashes (D61), so there is nothing re-derivable to print. Virtual replays are dropped — a replay has no desk — and a disqualified row is kept, because a slip is a seat allocation and not an award. Cards are ordered by display name, so two prints of one room are the same sheet. Available at any hour, before the contest as much as after: the whole point is the stack you cut up the night before. Cached for 60 s on a hash of the document.
         */
        get: {
            parameters: {
                query?: never;
    ...
    "/users": {
        ...
        /**
         * Search users by username or display name word (signed in)
         * @description **Requires an actor — a session, or a token carrying `users:read` (D188).** This route was public until F-52, and an anonymous caller could page every account on the judge: five requests at `limit=100` took the whole roster off the live host, with no credential and no meter. On a province’s judge that list is every pupil’s real name, most of them children, so the ruling is that bulk enumeration is a thing you have to be someone to do. Individual visibility is untouched: `GET /users/{username}`, its progress and its rating are still public, because that is how a competitive-programming judge works. Not `@SessionOnly()` — this is an API as well as a website, and a token is a named, revocable principal, which is exactly what an anonymous caller is not. The WALK is metered per account: a request carrying `cursor` spends one of twenty pages per hour, and a request without one spends nothing, so a search box (which never sends a cursor) can never be locked out by the meter. The key is the account, never the address — a school computer room is one NAT address and thirty pupils, and an IP-keyed meter would hand the room one budget between them and shut the last arrivals out mid-contest.
         */
        get: {
            parameters: {
                query?: {
            ...
            responses: {
                ...
                /** @description Not signed in (`authentication_required`). Deliberately the same refusal `GET /submissions` answers, and deliberately not a 403 or an empty page. */
                401: {
                    headers: {
                        [name: string]: unknown;
                    };
                ...
                /** @description Too many pages of the list have been walked by this account (`user_walk_rate_limited`) — twenty per hour, counting only requests that carry a `cursor` (D188). `Retry-After` carries the whole seconds until the oldest page falls out of the window. A refused request records nothing, so the window drains rather than pinning a caller against it. */
                429: {
                    headers: {
                        [name: string]: unknown;
                    };
    ...
    "/orgs/{slug}/teams": {
         ...
         * @description An owner or admin of the organization (or a global admin) sees every team; anybody else sees the teams they are ON, and a caller who can see the organization but belongs to no team in it gets an EMPTY page rather than a refusal — D66’s shape, for D66’s reason. An organization the caller may not see is 404. **Newest first (D177)**: the page is ordered by `teams.id` DESCENDING, and `cursor` is therefore an UPPER bound — the id of the last row of the previous page. The id is unique and monotonic, so the walk needs no tiebreak and can neither skip nor repeat a team. A cursor this list could not have issued is 422 `invalid_cursor`.
         */
        get: {
            parameters: {
                query?: {
